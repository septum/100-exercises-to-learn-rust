...
pub fn sum(v: Vec<i32>) -> i32 {
    let mut fv = v.clone();
    let lv = fv.split_off(v.len() / 2);

    let fjh = thread::spawn(move || fv.into_iter().sum::<i32>());
    let ljh = thread::spawn(move || lv.into_iter().sum::<i32>());

    fjh.join().unwrap() + ljh.join().unwrap()
}
